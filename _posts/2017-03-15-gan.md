...
In the BEGAN framework the authors come up with a convergence measure $ M_{global} = L(X) + \mid \gamma L(X) - L(G(z)) \mid $ where $\gamma$ is a hyperparameter between zero and one. The following plot shows that this convergence measure is well
correlated with image quality, we can also notice that convergence is really quick.

![_config.yml]({{ site.baseurl }}/images/convergence_measure.png)
<sup>**Fig. 3. Convergence measure versus number of iteration. This figure show that convergence is correlated
