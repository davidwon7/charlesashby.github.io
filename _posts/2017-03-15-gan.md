...
In the BEGAN framework the authors come up with a convergence measure $ M_global = L(X) + | \gamma L(X) - L(G(z)) | $ that is well correlated with image quality as can be seen in the following plot.
