...
network [7], I begin by introducing a vanilla Deep Convolutional GAN (DCGAN) [1][1]
and then move on to the new Boundary Equilibrium GAN (BEGAN) 
architecture [2][2].

I also gave a shot at Wasserstein GAN (WGAN) [4][4], but unlike [Louis Henri Frank's](https://github.com/louishenrifranc/ImageFilling), my results were quite disappointing, had time not
been an issue it would have been interesting to take a look at the [Improved training of Wasserstein GANs paper](https://arxiv.org/pdf/1704.00028.pdf) [5][5], but implementing it in Theano proved to be quite hard.
...
of the SqueezeNet architecture [3][3] and a decoder made up of multiple
...
I decided to encode the captions using a pretrained skip-thought model [6][6], this network learns,
...
the Lipschitz conditions like in Wasserstein GAN [4][4],
